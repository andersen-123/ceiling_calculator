...
class ImportService {
  ...
  static Future<File> saveImportedQuotes(
      List<Map<String, dynamic>> importedQuotes) async {
    ...
    for (int i = 0; i < importedQuotes.length; i++) {
      ...
      for (final item in lineItems) {
        buffer.writeln(
            '   - ${item.description} (${item.quantity} ${item.unit} × ${item.price} ₽ = ${item.amount} ₽)');
      }
    }
